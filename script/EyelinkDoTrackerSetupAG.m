...
if ~isempty(el.callback)
    if Eyelink('IsConnected') ~= el.notconnected
        if ~isempty(el.window)
            ...
            %% below was modified by AMG to make the calibration area smaller.
            %rect = Screen(el.window,'Rect'); %the original code.
            pctScale = .6;
            pS = [.5 - pctScale/2, .5 + pctScale/2];
            rect_o=Screen(el.window,'Rect');
            rect = [rect_o(1)+rect_o(3)*pS(1), rect_o(2)+rect_o(4)*pS(1), rect_o(1)+rect_o(3)*pS(2), rect_o(2)+rect_o(4)*pS(2)];
            %%
